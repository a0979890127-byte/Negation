...
"""
    Loki module for NEG_BEFORE_EXPLETIVE_MEI
...
INTENT_NAME = "NEG_BEFORE_EXPLETIVE_MEI"
...
MODULE_DICT = {
    "Account": import_from_path("Negation_lib_Account", os.path.join(os.path.dirname(CWD_PATH), "lib/Account.py")),
    "LLM": import_from_path("Negation_lib_LLM", os.path.join(os.path.dirname(CWD_PATH), "lib/LLM.py"))
}
"""
Account 變數清單
[變數] BASE_PATH         => 根目錄位置
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "張三沒上大學以前，他就已經很懂事了":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "李小姐沒結婚以前，他是絕對不會生小孩的":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("他沒來以前我就走了", "他沒來以前我就走了", [], {}, {})
    pprint(resultDICT)