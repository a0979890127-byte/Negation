...
MODULE_DICT = {
    "Account": import_from_path("Negation_lib_Account", os.path.join(CWD_PATH, "Account.py"))
}
"""
Account 變數清單
[變數] BASE_PATH         => 根目錄位置
    ...
    for x in sum(atkResult["result_obj"], []):
        ...
        if x["pos"] in ["TIME_holiday", "TIME_justtime", "TIME_day", "TIME_week", "TIME_month", "TIME_season", "TIME_year", "TIME_decade"]:
            if "time" in featureLIST:
                wordLIST.append(x["text"])
        if x["pos"] in ["LOCATION", "KNOWLEDGE_addTW", "KNOWLEDGE_routeTW"]:
        ...
        if x["pos"] in ["IDIOM"]:
            if "idiom" in featureLIST:
                wordLIST.append(x["text"])
        if x["pos"] in ["KNOWLEDGE_chemical"]:
            if "chemical" in featureLIST:
                wordLIST.append(x["text"])
